...
class PengirimanSampelController extends Controller
{
    ...
    {
        $pengirimanSampels = PengirimanSampel::with('sampel')
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'sampel_id' => 'required|exists:sampels,id',
        ...
        try {
            ...
            $sampel = Sampel::findOrFail($validated['sampel_id']);

            $totalhpp = $validated['jumlah'] * $sampel->harga;
    ...
    {
        $pengirimanSampel->load('sampel');
        return view('pengiriman-sampels.show', compact('pengirimanSampel'));
    }
    ...
    public function getTotalHpp(Request $request)
    {
        ...
        if ($sampelId && $jumlah) {
            ...
            if ($sampel) {
                $totalhpp = $jumlah * $sampel->harga;
                return response()->json(['totalhpp' => $totalhpp]);
            }
        }

        return response()->json(['totalhpp' => 0]);
    }
    ...
    public function rekap(Request $request)
    {
        ...
        // Query data rekap
        $rekapData = PengirimanSampel::with('sampel')
        ...
        $totalJumlah = $rekapData->sum('jumlah');
        $totalHpp = $rekapData->sum('totalhpp');
        $totalOngkir = $rekapData->sum('ongkir');
        $totalBiaya = $rekapData->sum('total_biaya');

        // Rekap per sampel
        $rekapPerSampel = $rekapData->groupBy('sampel_id')->map(function ($items, $sampelId) {
            $sampel = $items->first()->sampel;
            return [
