        ...
        <div class="pc-content">
            <div class="card shadow border-0" style="border-radius: 15px; overflow: hidden;">
                <div class="card-header border-0 text-white d-flex justify-content-between align-items-center"
                    ...
                    <div>
                        <h5 class="mb-0 text-white"><i class="fas fa-layer-group me-2"></i> Input Batch Transaksi Supplier</h5>
                        <small class="text-white text-opacity-75">Input jumlah barang per tanggal (7 Hari Sekali)</small>
                    </div>
                    <div class="badge bg-white text-warning py-2 px-3 shadow-sm"
                        style="font-size: 0.85rem; border-radius: 20px; border: 1px solid #f5a623;">
                        <i class="fas fa-tag me-1"></i> Harga: <b>HPP</b>
                    </div>
                </div>
                <div class="card-body">
                    <form action="{{ route('supplier_transactions.batch_store') }}" method="POST">
                        @csrf
                        <input type="hidden" name="supplier_id" value="{{ $supplier->id }}">

                        <div class="mb-4 d-flex justify-content-between align-items-center bg-light p-3 rounded">
                            <div>
                                <h6 class="mb-0 fw-bold">Supplier: {{ $supplier->nama }}</h6>
                                <p class="text-muted mb-0 small">Silakan isi jumlah barang pada kolom tanggal yang sesuai.</p>
                            </div>
                            <div class="text-end">
                                <span class="badge bg-soft-warning text-warning border border-warning px-3 py-2">
                            ...
                            <table class="table table-bordered table-hover sticky-header" id="batchTable">
                                <thead class="table-light text-center">
                                    <tr>
                                        ...
                                        <th rowspan="2" class="align-middle" style="min-width: 120px;">HPP (Rp)</th>
                                        <th colspan="5">Jumlah Masuk (Pcs)</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end hpp-val" data-hpp="{{ $barang->hpp ?? 0 }}">
                                ...
                                <tfoot class="table-light">
                                    <tr class="fw-bold">
                                        <td colspan="2" class="text-center">Total Belanja Per Tanggal</td>
                                        @foreach($dates as $date)
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4">
                            ...
                            <button type="submit" class="btn btn-warning text-dark fw-bold px-4 shadow">
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function updateTotals() {
                ...
                document.querySelectorAll('#batchTable tbody tr').forEach(row => {
                    const hpp = parseFloat(row.querySelector('.hpp-val').dataset.hpp) || 0;
                    let rowSumQty = 0;
